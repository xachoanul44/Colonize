...
if flag == true{
	for(i = 0; i < w; i += 1) { 
	   for(j = 0; j < h; j += 1) {
	      ...
	      if (Spr[i, j] != -1) draw_sprite_ext(Spr[i, j], -1, i * long + _x + (long / 2), j * long + _y + (long / 2), 0.5, 0.5, image_angle, c_white, 1);
	      if (Amount[i, j] > 1) draw_text_transformed(i * long + _x + (long / 2), j * long + _y + (long / 2), Amount[i, j], 1, 1, image_angle);
	   } 
	}
}else{
	exit
